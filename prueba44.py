...
def login_facial():
    ...
    while True:
        ...
        advertencias = []
        if resultados:
            rostro_principal = max(resultados, key=lambda r: r['box'][2]*r['box'][3])
            box = rostro_principal['box']
            rostro = limpiar_recorte_rostro(frame, box)
            if rostro is not None:
                if rostro_con_objeto_o_mascarilla(rostro):
                    advertencias.append("Mascarilla/objeto detectado")
                if detectar_lentes(rostro):
                    advertencias.append("Lentes detectados")
                if advertencias:
                    mensaje = " / ".join(advertencias)
                    rostro_valido = False
                else:
                    mensaje = "Rostro listo para login (presiona ESC)"
                    rostro_valido = True
                    frame_guardar = frame.copy()
                x, y, w, h = box
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                cv2.putText(frame, mensaje, (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255) if not rostro_valido else (0,255,0), 2)
            else:
                mensaje = "No se pudo recortar el rostro (fuera de límites o muy pequeño)"
                rostro_valido = False
                cv2.putText(frame, mensaje, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)
        else:
    ...
    if rostro_valido and frame_guardar is not None:
        cv2.imwrite(usuario_login+"LOG.jpg", frame_guardar)
        usuario_entrada2.delete(0, tk.END)
        contra_entrada2.delete(0, tk.END)
        # Aquí continúa tu lógica de comparación de rostros...
        #----------------- Funcion para guardar el rostro --------------------------
        def log_rostro(img, lista_resultados):
            data = pyplot.imread(img)
            for i in range(len(lista_resultados)):
        ...
        Label(pantalla2, text="No se permite login con mascarilla, lentes, objeto o sin rostro visible", fg="red", font=("Calibri", 11)).pack()